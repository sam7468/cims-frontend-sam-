...
function UpdateForm(props) {
  ...
  const { fields, initialContacts, initialFormValues2, addressFields } =
    UseForm();
  ...
  useEffect(async () => {
    ...
    await axios
      ...
      .then((clientInfo) => {
        setformData(clientInfo.data.data[0]);
        setN(Object.keys(clientInfo.data.data[0].contacts).length);
  ...
  const setformvalue = (e) => {
    ...
    if (
      ...
      if (n === 3 ? true : Object.keys(new_form.contacts).length <= n) {
        setAddOthers(true);
      } else {
        setAddOthers(false);
      }
  ...
  const addressField = addressFields.map((field) => {
        ...
        <Grid item xs={12} sm={gridStyle}>
          <FormControl size="small" className={classes.formControl}>
            ...
            <Select
              labelId="country"
              Inputprops={
                !editMode && { readOnly: true, disableUnderline: false }
              }
              name={field.name}
    ...
    if (
      field.name === "district" &&
      formData.pincode !== "" &&
      Object.keys(loc.loc["districts"])[0].length > 1
    ) {
      return (
        <Grid item xs={12} sm={gridStyle}>
          <FormControl size="small" className={classes.formControl}>
            ...
            >
              {Object.keys(loc.loc["districts"]).map((key) => (
                <MenuItem disabled={!editMode} key={key} value={key}>
                  {key}
                </MenuItem>
              ))}
            </Select>
    ...
    if (
      ...
      formData.district !== "" &&
      formData.pincode !== "" &&
      loc.loc["districts"][formData["district"]]
        ? loc.loc["districts"][formData["district"]].length > 1
        : false
    ) {
      return (
        <Grid item xs={12} sm={gridStyle}>
          <FormControl size="small" className={classes.formControl}>
            ...
            >
              {loc.loc["districts"][formData["district"]].map((dist) => (
                <MenuItem disabled={!editMode} key={dist} value={dist}>
                  {dist}
                </MenuItem>
              )) || ""}
            </Select>
              ...
              <Grid container item xs={6} direction="column">
                <div>
                  ...
                  <TextField
                    ...
                    variant="outlined"
                    InputProps={
                      !editMode && { readOnly: true, disableUnderline: true }
                    }
                    name="brandname"
                ...
                <div>
                  ...
                  <Box sx={{ minWidth: 120 }}>
                    <FormControl fullWidth>
                      ...
                      >
                        <MenuItem disabled={!editMode} value={"client 1"}>
                          client 1
                        </MenuItem>
                        <MenuItem disabled={!editMode} value={"client 2"}>
                          client 2
                        </MenuItem>
                        <MenuItem disabled={!editMode} value={"client 3"}>
                          client 3
                        </MenuItem>
                      </Select>
                    </FormControl>
